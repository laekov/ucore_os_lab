...
alloc_proc(void) {
     ...
     */
     //LAB5 2016011279 : (update LAB4 steps)
    /*
	 ...
	 */
     ...
     //LAB6 2016011279 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2016011279 HINT:need add some code to init fs in proc_struct, ...
		proc->filesp = 0;
    }
    return proc;
}
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    //LAB8:EXERCISE2 2016011279 HINT:how to copy the fs in parent's proc_struct?
    /*
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2016011279  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
	...
	for (phnum = 0; phnum < elf->e_phnum; ++ phnum) {
		...
		if ((ret = mm_map(mm, ph->p_va, ph->p_memsz, vm_flags, 0))) {
			goto bad;
		}
	...
	if ((ret = mm_map(mm, USTACKTOP - USTACKSIZE, USTACKSIZE, vm_flags, 0))) {
		goto bad;
	}
	mm_count_inc(mm);
	current->mm = mm;
	...
	uintptr_t stacktop = USTACKTOP - (argvs / sizeof(long) + 1) * sizeof(long);
	char **uargv = (char**)(stacktop - argc * sizeof(char*));
	argvs = 0;
	for (i = 0; i < argc; ++ i) {
		uargv[i] = strcpy((char*)(stacktop + argvs), kargv[i]);
		argvs += strnlen(kargv[i], EXEC_MAX_ARG_LEN + 1) + 1;
	}
